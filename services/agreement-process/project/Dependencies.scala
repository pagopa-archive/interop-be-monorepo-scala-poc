...
object Dependencies {
  ...
  object Jars {
    ...
    lazy val client: Seq[ModuleID]    =
      Seq(akka.stream, akka.http, akka.httpJson4s, akka.slf4j, json4s.jackson, json4s.ext).map(
        _ % Compile
      )
    lazy val lifecycle: Seq[ModuleID] = Seq(cats.core % Compile, scalatest.core % Test)
    lazy val events: Seq[ModuleID]    = Seq()
  }
}
