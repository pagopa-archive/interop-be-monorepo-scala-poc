...
object Dependencies {
  ...
  object Jars {
    ...
    val models: Seq[ModuleID] = Seq(spray.core, cats.core).map(_ % Compile)

    lazy val generated: Seq[ModuleID] =
      Seq(akka.stream, akka.http, akka.httpJson4s, akka.slf4j, logback.classic, spray.core, mustache.mustache).map(
        _ % Compile
      )
