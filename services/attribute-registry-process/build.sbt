...
val attributeRegistryManagementClient = ProjectRef(file("../attribute-registry-management"), "client")
...
lazy val client = project
  .in(file("client"))
  .settings(
    name                := "interop-be-attribute-registry-process-client",
    scalacOptions       := Seq(),
...
lazy val utils = project
  .in(file("utils"))
  .settings(name := "interop-be-attribute-registry-process-utils", scalafmtOnCompile := true, sharedSettings)
...
lazy val root = (project in file("."))
  ...
  .dependsOn(
    utils,
    attributeRegistryManagementClient,
